  ...
  <div>
    ...
    <swiper ref="mySwiper" :options="swiperOption">
      ...
      <swiper-slide v-for="(tab,i) in tabs" :key="i">
			<mescroll-vue :ref="autoMescrollI('mescroll',i)" :aa="autoMescrollI('mescroll',i)" :down="getMescrollDown(i)" :up="getMescrollUp(i)" @init="mescrollInit(i,arguments)">
        <div :id="autoMescrollI('dataList',i)">
          ...
          <div v-for="pd in tabs[i].list" :key="pd.id">
              <goodLists :listData = 'pd'></goodLists>  
          </div>
...
import {getGoodsNav,getGoodsList} from '@/api/goods.js'

import goodLists from './goodList/index.vue'
...
export default {
  ...
  methods: {
    getinfo(){
      let self = this;
      getGoodsNav({}).then(reponse=>{
        self.tabs = reponse.data;
      })
      
    },
	 autoMescrollI(tip,index){
        return tip+index;
	 },  
  ...
  //   // console.log(vm.$refs)
  //      curMescroll.beforeRouteEnter() // 进入路由时,滚动到原来的列表位置,恢复回到顶部按钮和isBounce的配置  
  //     // 恢复水平菜单的滚动条位置
  ...
  beforeRouteLeave (to, from, next) { // 如果没有配置回到顶部按钮或isBounce,则beforeRouteLeave不用写
	...
	curMescroll[0].beforeRouteLeave();
    next()
  }
}
</script>
